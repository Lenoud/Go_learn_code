...
func Login(username, passwd string) bool {
	if login_user == "" {
		//获取用户文件中的信息
		data, _ := os.ReadFile("userinfo.json")

		err := json.Unmarshal(data, &userInfo)
		if err != nil {
			fmt.Println("读取用户文件错误:", err)
			return false
		}
		//登录判断逻辑
		if userInfo["name"] == username && userInfo["passwd"] == passwd {
			login_user = username
			//登录成功后直接GET所有商品数据
			data, err := os.ReadFile("shopinfo.json")
			if err != nil {
				fmt.Println("读取商品文件错误:", err)
			}
			// 解析 JSON 数据
			json.Unmarshal(data, &shop_list)
			return true
		} else {
			return false
		}
